...
1. Upload `newsfront-bbpress` to your `/wp-content/plugins/` directory
...
For example: the Genesis breadcrumbs are removed using `remove_action( 'radium_before_loop', 'radium_do_breadcrumbs' );` - however if they are not hooked into radium_before_loop then they will not be removed as they should be.
